...
HMATS = randi([10 40],3,1)
format shortG
LMAS = round(.1 + (3)*rand(3,1), 2, 'significant')

trait = [LMAS(1), HMATS(1); LMAS(2), HMATS(2); LMAS(3), HMATS(3)]; %3 trait combinations
trait_str= string(trait);
timesteps=100; 
type=length(trait);
maxindiv =20; %maximum individuals that can be modeled

filename = "randomtraits.gif"
...
nexttile(1);
set(gca, 'Ylim',[0 40])
xlabel('Offspring count');
LMA1 = sprintf('LMA %g',trait_str(1,1)); 
HMAT1 = sprintf(' HMAT %g', trait_str(1,2));
trait1 = append(LMA1, HMAT1)
annotation('textbox', [0.14, 0.75, 0.1, 0.1], 'String', trait1);
hold on

nexttile(2);
set(gca, 'Ylim', [0 40]);
xlabel('Offspring count');
LMA2 = sprintf('LMA %g',trait_str(2,1)); 
HMAT2 = sprintf(' HMAT %g', trait_str(2,2));
trait2 = append(LMA2, HMAT2);
annotation('textbox', [0.44, 0.75, 0.1, 0.1], 'String', trait2);
hold on 

nexttile(3)
set(gca, 'Ylim', [0 40]);
xlabel('Offspring count');
LMA3 = sprintf('LMA %g',trait_str(3,1)); 
HMAT3 = sprintf(' HMAT %g', trait_str(3,2));
trait3 = append(LMA3, HMAT3);
annotation('textbox', [0.74, 0.75, 0.1, 0.1], 'String', trait3);
hold on
